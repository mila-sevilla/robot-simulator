...
function model.dlg.onAcceptPalletEdit(arg1)
    sim.auxFunc('enableRendering')
    model.writeInfo_onePallet(model.dlg.selectedPalletHandle,arg1)
...
function model.dlg.editPalletClick_callback()
    if model.dlg.selectedPalletHandle>=0 then
        ...
        local palletData=model.readInfo_onePallet(model.dlg.selectedPalletHandle)
        local data={}
    ...
    if model.dlg.selectedPalletHandle>=0 then
        ...
        if #value>0 then
            ...
            if model.getPalletWithName(value)==-1 then
                ...
                local data=model.readInfo_onePallet(model.dlg.selectedPalletHandle)
                data.name=value
                model.writeInfo_onePallet(model.dlg.selectedPalletHandle,data)
                model.updatePluginRepresentation_onePallet(model.dlg.selectedPalletHandle)
