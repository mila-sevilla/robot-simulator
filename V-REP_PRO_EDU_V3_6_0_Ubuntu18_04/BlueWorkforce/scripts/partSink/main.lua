local isCustomizationScript=sim.getScriptAttribute(sim.getScriptHandle(sim.handle_self),sim.scriptattribute_scripttype)==sim.scripttype_customizationscript

if not sim.isPluginLoaded('Bwf') then
    function sysCall_init()
    end
else
    function sysCall_init()
        model={}
        simBWF.appendCommonModelData(model,simBWF.modelTags.PARTSINK)
        if isCustomizationScript then
            -- Customization script
            if model.modelVersion==1 then
                require("/BlueWorkforce/scripts/partSink/common")
                require("/BlueWorkforce/scripts/partSink/customization_main")
                require("/BlueWorkforce/scripts/partSink/customization_data")
                require("/BlueWorkforce/scripts/partSink/customization_dlg")
                require("/BlueWorkforce/scripts/partSink/customization_ext")
            end
        else
            -- Child script
            if model.modelVersion==1 then
                require("/BlueWorkforce/scripts/partSink/common")
                require("/BlueWorkforce/scripts/partSink/child_main")
            end
        end
        sysCall_init() -- one of above's 'require' redefined that function
    end
end
