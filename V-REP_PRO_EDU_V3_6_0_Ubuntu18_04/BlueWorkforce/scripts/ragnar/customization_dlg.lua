...
function model.dlg.dynamicsChange_callback(uiHandle,id,newValue)
    local c=model.readInfo()
    newValue=tonumber(newValue)
    if newValue then
...
function model.dlg.refresh()
    if model.dlg.ui then
        ...
        simUI.setEditValue(model.dlg.ui,13,simBWF.format("%.0f",c['dynamics']*180/math.pi),true)
...
function model.dlg.createDlg()
    ...
    <tab title="General">
            <group layout="form" flat="false">
                ...
                <edit on-editing-finished="model.dlg.dynamicsChange_callback" id="13"/>
            </group>
            <group layout="form" flat="false">
    ...
    <tab title="Pick/Place">
            ...
            <group layout="form" flat="false">
                ...
                <label text="Pick also without target in sight (override)" style="* {background-color: #ccffcc}"/>
