...
function model.readInfo()
    ...
    if not data['bitCoded'] then
        data['bitCoded']=1 -- 1=hidden during sim, 2=calibration balls hidden during sim, 4=showPts, 8=create parts (online mode), 16=startStopLine enable, 32=pick also without target in sight
    end
    ----------------------------------------------------------------------------------------------
    data['palletId']=nil
...
model.objRefJobInfo={5} -- information about jobs stored in object references. Item 1 is where job related obj refs start, other items are obj ref indices that are in the job scope
...
if isPick then
    ...
    model.handles.startLineBox=sim.getObjectHandle('pickTrackingWindow_startLine@silentError')
    ...
    model.handles.startLineBox=sim.getObjectHandle('placeTrackingWindow_startLine@silentError')
