function model.getIsPickWithoutTargetOverridden()
    local associatedRobot=model.getAssociatedRobotHandle()
    if associatedRobot>=0 then
        local overridden=simBWF.callCustomizationScriptFunction('model.ext.isPickWithoutTargetOverridden',associatedRobot,model.handle)
        return overridden
    end
    return false
end
