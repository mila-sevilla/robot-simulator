...
function model.dlg.updateEnabledDisabledItems()
    if model.dlg.ui then
        ...
        if model.isPick then
            simUI.setEnabled(model.dlg.ui,6,true and not model.getIsPickWithoutTargetOverridden(),true)
        end
    end
end
...
function model.dlg.refresh()
    if model.dlg.ui then
        ...
        simUI.setEditValue(model.dlg.ui,23,simBWF.format("%.0f",d/0.001),true)
        ...
        if model.isPick then
            ...
            if model.getIsPickWithoutTargetOverridden() then
                val=true
            end
            simUI.setCheckboxValue(model.dlg.ui,6,simBWF.getCheckboxValFromBool(val),true)
        end
...
function model.dlg.pickWithoutTarget_callback(ui,id,newVal)
    local c=model.readInfo()
...
function model.dlg.calibrationBallDistanceChange_callback(ui,id,newVal)
    ...
    if v then
        ...
        if v~=c['calibrationBallDistance'] then
            ...
            model.writeInfo(c)
            model.alignCalibrationBallsWithInput()
            model.updatePluginRepresentation()
            simBWF.markUndoPoint()
        end
    end
    ...
    if (not model.dlg.ui) and simBWF.canOpenPropertyDialog() then
        ...
        <tabs id="77">
            <tab title="General">
                ...
                <edit on-editing-finished="model.dlg.calibrationBallDistanceChange_callback" id="23"/>]]
        if model.isPick then
            xml=xml..[[
                <label text="Pick also without target" style="* {background-color: #ccffcc}"/>
                <checkbox text="" on-change="model.dlg.pickWithoutTarget_callback" id="6" />]]
        end
        xml=xml..[[
            </group>
            ...
            <group layout="form" flat="false">
                ...
                <checkbox text="Stop/start line (mm)" style="* {background-color: #ccffcc}" on-change="model.dlg.startStopLine_callback" id="50" />
                <label text=""/>
