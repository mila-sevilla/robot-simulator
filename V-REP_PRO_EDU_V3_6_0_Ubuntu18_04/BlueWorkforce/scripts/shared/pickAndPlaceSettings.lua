...
function model.pickPlaceDlg.refresh()
    if model.pickPlaceDlg.pickPlaceData.ui then
        ...
        simUI.setEditValue(model.pickPlaceDlg.pickPlaceData.ui,3,simBWF.format("%.0f",model.pickPlaceDlg.pickPlaceData.dynamics*100),true)
...
function model.pickPlaceDlg.dynamicChange_callback(ui,id,newValue)
    newValue=tonumber(newValue)
    if newValue then
        if newValue<0 then newValue=0 end
        if newValue>100 then newValue=100 end
        newValue=newValue/100
        if newValue~=model.pickPlaceDlg.pickPlaceData.dynamics then
            model.pickPlaceDlg.pickPlaceData.dynamics=newValue
        end
    end
    model.pickPlaceDlg.refresh()
end
...
function model.pickPlaceDlg.display(settingsData,title,forGripper,applyCallback,closeCallback,previousDlgPos)
    ...
    xml=xml..[[
                ...
                <label text="Dynamics (%)"/>
                <edit on-editing-finished="model.pickPlaceDlg.dynamicChange_callback" id="3"/>
